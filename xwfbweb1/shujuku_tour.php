...
<html>
  ...
  <body>
    ...
    <div class="row"><!--导航栏之下-->
        ...
        <div class="side"><!--左边栏-->
            <h1>搜索</h1><br/>
                <form action="search_tour.php" method="post">
        ...
        <div class="main"><!--页面主体（右）-->
            ...
            <h1>旅行</h1>
                ...
                while($row = $result->fetch_assoc()) {
                    echo "<tr> <td>" .$row["news_id"].    "</td>   <td><a href='neirong_tour.php?id={$row['news_id']}'>"  .$row["title"].     "</a></td>   <td> "  .$row["author"].   "</td>    <td>  ".$row["source"]."  </td>  <td>  ".$row["type"]."  </td> <td>  ".$row["pubtime"]."  </td>
                <td>
                    <a href='add.php?id={$row['news_id']}'><button>添加</button></a>
                    <a href='delete.php?id={$row['news_id']}'><button>删除</button></a>
                    <a href='update_tour.php?id={$row['news_id']}'><button>修改</button></a>
                    </td>
                    </tr>";
